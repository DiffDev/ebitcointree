...
uint256 hashGenesisBlock("0x000009b0345259f2cddb01cd73e63a7353bafd71d91b9fc148cdffc7643a61be");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EBITCOINTREE: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EB3COIN Signed Message:\n";
...
{
    // EBITCOINTREE: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 5 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 5 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 5 * 2.8;
...
{
    // EBITCOINTREE: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		const char* pszTimestamp = "eb3 genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1488399128;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2023033;
        ...
        assert(block.hashMerkleRoot == uint256("0xd0b58b85041736eeb7f6cdb8e479e36b89aa3f6f008afa56525aae1ee243a659"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xba, 0xde }; // EBITCOINTREE: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EBITCOINTREEMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EBITCOINTREEMiner : ProcessBlock, block not accepted");
    }
